...
object NaturalNumbers {
  ...
  class S[N <: Nat] extends Nat {
    type Add[A <: Nat] = N + S[A]                            // S(N) + A = N + S(A)
    type SubFrom[A <: Nat] = A#Pre - N                       // A - S(N) = A#Pre - N
    type Mult[A <: Nat] = A + (N * A)                        // S(N) * A = A + (N * A)
    type Div[A <: Nat] = S[N]#IfLt[A, _0, S[S[N] - A / A]]   // S(N) / A = S((S(N) - A )/ A) if S(N) >= A
